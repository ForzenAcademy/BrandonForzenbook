...
import com.brandon.appjava.network.user.CreateUserResponseJava;
        ...
        try {
            ...
            if (response.code() == USER_DUPLICATE_CODE) {
                ...
                try {
                    ...
                    if (Objects.equals(error.getReason(), INVALID_FIRST_NAME)) throw new InvalidFirstNameExceptionJava(INVALID_FIRST_NAME);
                    if (Objects.equals(error.getReason(), INVALID_LAST_NAME)) throw new InvalidLastNameExceptionJava(INVALID_LAST_NAME);
                    if (Objects.equals(error.getReason(), INVALID_EMAIL)) throw new InvalidEmailExceptionJava(INVALID_EMAIL);
                    if (Objects.equals(error.getReason(), INVALID_LOCATION)) throw new InvalidLocationExceptionJava(INVALID_LOCATION);
                    if (Objects.equals(error.getReason(), INVALID_DATE_OF_BIRTH)) throw new InvalidDateExceptionJava(INVALID_DATE_OF_BIRTH);
